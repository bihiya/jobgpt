"""Apply jobs worker using Playwright adapters."""
...
class ApplyWorker(BaseWorker):
    ...
    async def handle(self, topic: str, payload: dict[str, Any]) -> None:
        ...
        answers = {
            "years": str(user.profile.experience_years) if user else "0",
            "notice": str(user.profile.notice_period_days) if user else "0",
            "location": user.profile.location if user else "",
        }
